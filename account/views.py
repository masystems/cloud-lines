...
def get_stripe_connected_account_links(request, attached_service):
    ...
    vars:
    stripe_account = django model object for storing stripe connected account data
    account_link = url to send user to for creating/editing their account
    stripe_package = connected stripe account data, used to check for errors
    edit_account = url used to edit their account
    :return:
    """
    stripe_account, account_link, stripe_package, edit_account, account_link_setup = None, None, None, None, None
    stripe.api_key = get_stripe_secret_key(request)
    ...
    if request.user == attached_service.user.user:
        ...
        stripe_package = stripe.Account.retrieve(stripe_account.stripe_acct_id)

        if stripe_account.stripe_acct_id:
            try:
                # get edit account link
                edit_account = stripe.Account.create_login_link(stripe_account.stripe_acct_id)
    ...
    return stripe_account, account_link, stripe_package, edit_account, account_link_setup


@login_required(login_url='/accounts/login/')
def create_package_on_stripe(request, stripe_account):
    # get strip secret key
    # import stripe key
    public_api_key = get_stripe_secret_key(request)

    if not stripe_account.stripe_acct_id:
    ...
    if not stripe_account.bn_stripe_product_id:
        # create product
        product = stripe.Product.create(name='Birth Notification',
                                        stripe_account=stripe_account.stripe_acct_id)
        stripe_account.bn_stripe_product_id = product.id
    # pedigree reg product
    ## todo!

    stripe_account.save()

    return get_account_link(request, stripe_account)


def get_account_link(request, stripe_account):
    account_link = stripe.AccountLink.create(
        account=stripe_account.stripe_acct_id,
        refresh_url=f'{django_settings.HTTP_PROTOCOL}://{request.META["HTTP_HOST"]}/birth_notification',
        return_url=f'{django_settings.HTTP_PROTOCOL}://{request.META["HTTP_HOST"]}/birth_notification',
