...
class BirthNotificationForm(forms.ModelForm):
    class Meta:
        ...
        fields = '__all__'
        exclude = ('account', 'user', 'attached_bolton', 'births', 'stripe_payment_token', 'stripe_payment_source', 'paid', 'complete')
        help_texts = {
        ...
        widgets = {
            'dob': forms.DateInput(format='%d/%m/%Y', attrs={'type':'date', 'required':''}),
